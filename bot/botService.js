...
const calculateTimeIntervals = (startTime, endTime) => {
    ...
    // Calculate intervals at 20%, 40%, 60%, and 80% of total contest duration
    return [
        totalTime * 0.2,
        totalTime * 0.4,
        totalTime * 0.6,
...
const fillSlotsWithBots = async (contestId) => {
    try {
        ...
        // Calculate the max bots to add based on 15% of available slots, capped at available slots
        const maxBots = Math.min(Math.floor(contest.availableSlots * 0.15), contest.availableSlots);
        let botsFilled = 0;
